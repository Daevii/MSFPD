...
<script>
  $(function() {
    $('.approve_invoice').click(function() {
      // Get the Receipt Number from the clicked button
      const receiptNumber = $(this).closest('tr').find('td:eq(2)').text();

      // Get the invoice ID from the clicked button
      ...
      swal({
        ...
        if (willDelete) {
          ...
          swal("Order Accepted", {
            text: "The Invoice Have Been Approved",
            icon: "success",
          }).then(function() {
            location.reload();
          });
